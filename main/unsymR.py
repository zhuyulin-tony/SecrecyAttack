...
parser.add_argument('--dataset', type=str, default='word', choices=['word', 'bitcoin_alpha', 'bitcoin_otc'], help='dataset')
...
for t in [1,2,3,4,5]:
    ...
    save_dir = root_dir+'/'+args.dataset+'/unsymR/'+str(t)
    try:
    ...
    class gradmax(nn.Module):
        ...
        def inner_train(self, triple):
            ...
            R_t = self.model.signed_autocovariance_matrix(triple)
            for epoch in range(self.iters):
                loss = torch.dist(self.model.embs @ self.model.embs.T, R_t)
                loss.backward()
                self.optimizer.step()
                self.optimizer.zero_grad()
                #print('t:', t, 'epoch:',epoch,'loss:',loss.item())
        ...
        def get_meta_grad(self, triple_copy):
            ...
            R_t = self.model.signed_autocovariance_matrix(triple_torch)
