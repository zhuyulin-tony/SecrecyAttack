...
for eta in eta_lst:
    for t in [1,2,3,4,5]:
        ...
        save_dir = root_dir+'/'+args.dataset+'/symR_polar_'+str(eta)+'/'+str(t)
        try:
        ...
        class gradmax(nn.Module):
            ...
            def get_meta_grad(self, triple_copy):
                ...
                atk_loss = -self.loss_fn(torch.clamp(0.5*(sims+1), 1e-3, 1.-1e-3), torch.from_numpy(self.test_label).to(self.device)) - eta * self.model.polar(triple_torch)
                atk_loss.backward()
