...
parser.add_argument('--device', type=str, default='cuda:4', choices=['cuda:1','cpu'], help='device')
...
lam_lst = [2]
#root_dir = 'E:/polyu/attack_signed_link_prediction/src'
###########################################################################
######################## load dataset #####################################
###########################################################################
for lam in lam_lst:
    for t in [1,2,3,4,5]:
        ...
        save_dir = root_dir+'/'+args.dataset+'/ols_tg_'+str(lam)+'/'+str(t)
        try:
        ...
        class gradmaxOLS(nn.Module):
            ...
            def get_meta_grad(self, triple_copy):
                ...
                for i, (x_test, y_test) in enumerate(self.test_loader): 
                    ...
                    atk_loss = -self.loss_fn(outputs, y_test.reshape(-1,1)) - (lam/2) * \
                               (1+(torch.trace(self.sparse_matrix_power(adj_mask))/torch.trace(self.sparse_matrix_power(adj_unsign))))
                    atk_loss.backward()
